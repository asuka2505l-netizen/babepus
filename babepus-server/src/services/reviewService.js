...
const createReview = async (reviewerId, payload) => {
  ...
  try {
    ...
    // Calculate new rating stats for the seller
    const [ratingStats] = await connection.query(
      `SELECT
        ROUND(COALESCE(AVG(rating), 0), 2) AS rating_average,
    ...
    await connection.query(
      `UPDATE users
       SET
        ...
        rating_count = ?
       WHERE id = ?`,
      [ratingStats[0].rating_average, ratingStats[0].rating_count, transaction.sellerId]
    );
